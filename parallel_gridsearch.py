# -*- coding: utf-8 -*-
"""
Created on Fri Sep 19 18:36:00 2025
...
class ParallelGridSearch:
    ...
    def __init__(self, X, y, rskf, scaler, model, param_grid, balancer=None,
                 ...
                 parallel_over="fold", save_models=True):
        """
        :param save_models: se False, non salva i modelli completi in memoria (riduce l'uso di RAM)
        """
        self.X = X
        self.y = y
        self.rskf = rskf
    ...
    def process_fold(self, fold_idx, train_idx, val_idx, param_comb=None):
        ...
        results = []
        param_combinations = [param_comb] if param_comb else self.param_grid
        ...
        for param in param_combinations:
            ...
            results.append(self.FoldResult(
                ...
                model=model if self.save_models else None,  # <-- non salva il modello se save_models=False
                y_pred=y_pred,
    ...
    def parallel_training(self):
        ...
        if self.parallel_over == "fold":
            # Parallelizza sui fold (come facevi prima)
            ...
            # Parallelizza sulle combinazioni di parametri
            all_results = Parallel(n_jobs=self.n_cores, backend="loky")(
    ...
    def _process_param_comb(self, param_comb):
        # Processa tutti i fold per una singola combinazione di parametri
        results = []
